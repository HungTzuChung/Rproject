...
```{r ans11}
transcriptData[transcriptData$學號=="edfeQ7fb8",1:9]->subsample
```
...
```{r ans12}
mutate(subsample,及格=subsample$學期成績 >= 60 )
summarise(subsample,
          sum(subsample$學分數) ,
sum(subsample$學分數[subsample$及格]) )
```
...
```{r ans16}
hundredUp<-transcriptData[,transcriptData$學年=="100"&&transcriptData$學期=="1"]
通識科目總表<-unique(hundredUp,科目名稱)
```
