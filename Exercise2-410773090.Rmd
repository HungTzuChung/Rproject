...
```{r ans1}
library(stringr)
str_sub(iPeenData$均消,5,end ="元"）->step1
as.numeric(step1)->step2
stringr::str_replace(",""")
```
...
```{r ans21}
lubridate::mdy_hms(filmVotingData$Timestamp)->answer1
```
...
```{r ans22}
library(lubridate)
ddd<-mdy_hms(filmVotingData$Timestamp,tz=UTC)
lubridate::with_tz(ddd,tz=Asia/Taipei)
```
...
```{r ans24}
as.factor(filmVotingData$Timestamp) -> filmVotingData$Timestamp
