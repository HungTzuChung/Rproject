...
```{r ans13}
colMeans(gsHW2ws[,1:5], na.rm=TRUE)
```
...
```{r ans16}
# 計算每人答對題數
gsHW2ws$答對題數 <- rowSums(gsHW2ws[,1:5],na.rm = TRUE)
# 取出答對4題的子樣本
sample4corrects<- gsHW2ws[(gsHW2ws$答對題數==4),1:5]
# 計算子樣本的各題答對率
...
gsHW2ws$答對題數 <- rowSums(gsHW2ws[,1:5],na.rm = TRUE)
# 取出答對4題的子樣本
sample3corrects<- gsHW2ws[(gsHW2ws$答對題數==3),1:5]
# 計算子樣本的各題答對率
...
```{r ans31}
cll<-locale(grouping_mark = ",",decimal_mark = ".")
exData$期間<-as.factor(exData$期間)
exData%>%
  mutate_at(
    vars(contains("/")),funs(parse_number(.,locale = cll)))->exData
exData
```
...
```{r ans32}
exData%>%
  mutate_at(vars(contains("/USD")),funs(1/.)
...
```{r ans33}
ll<-str_locate(colnames(exData), "/")
  str_sub(colnames(exData),1,ll-4)
```
