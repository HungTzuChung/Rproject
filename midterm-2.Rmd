...
```{r ans18}
hwGradesSubsample$平均成績[(hwGradesSubsample$女性==F)]->bbb
  mean(bbb)
```
...
hwGradesSubsample$平均成績[(hwGradesSubsample$女性==T & hwGradesSubsample$有團活==T)] ->tt
  mean(tt)-> meanGradeFemaleWithAct
...
with_tz(hwSubmit$Timestamp,tzone="Asia/Taipei")->hwSubmit$Timestamp
as.character(hwSubmit$學號)->hwSubmit$學號
hwSubmit
...
hwSubmit3$截止時差<-difftime(deadline3,hwSubmit3$Timestamp,units = "hours")
hwSubmit3
